...
try:
    ...
    def searchrecords():
       ...
       # messagebox.showinfo("message", strWhereStatement)
        spath = os.getcwd() + "\\" + "Shidduch.accdb"
        if path.exists(spath)==True:
            connectstr = 'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=%s;' % spath
        else:
            connectstr = 'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=C:\Temp\Shidduch.accdb'
        ...
        #if treeview has data delete rows
    ...
    tree.column('#0', stretch=tk.YES, minwidth=0,width=5)
    tree.column('#1', stretch=tk.YES,minwidth=0,width=100)
    tree.column('#2', stretch=tk.YES,minwidth=0,width=100)
    tree.column('#3', stretch=tk.YES,minwidth=0,width=50)
    tree.column('#4', stretch=tk.YES,minwidth=0,width=50)
    tree.column('#5', stretch=tk.YES,minwidth=0,width=50)
    tree.column('#6', stretch=tk.YES,minwidth=0,width=150)
    tree.column('#7', stretch=tk.YES,minwidth=0,width=100)
    tree.column('#8', stretch=tk.YES,minwidth=0,width=200)
